...
export const requestOrders = () => (dispatch) =>
  OrderAPIUtil.fetchOrders().then((orders) => dispatch(receiveOrders(orders)));

export const requestOrder = (orderId) => (dispatch) =>
  OrderAPIUtil.fetchOrder(orderId).then((order) =>
    dispatch(receiveOrder(order))
  );

export const createOrder = (order) => (dispatch) =>
  OrderAPIUtil.createOrder(order).then((order) =>
    dispatch(receiveOrder(order))
  );

export const updateOrder = (order, id) => (dispatch) =>
  OrderAPIUtil.updateOrder(order, id).then((order) =>
    dispatch(receiveOrder(order))
  );
